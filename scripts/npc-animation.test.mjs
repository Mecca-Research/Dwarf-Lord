...
test('all 14 non-player character profiles have eight angles and role-specific stance references', () => {
  ...
  for (const character of library) {
    ...
    assert.equal(manifest.frames.length, character.name === 'Borrin' ? 10 : 12);
...
test('Elder and Borrin use narrative poses rather than mislabeled labor cycles', () => {
  ...
  assert.deepEqual(poses('Borrin'), ['step-ledger','explain']);
});
