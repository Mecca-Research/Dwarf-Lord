...
export function WorldMatsProvider({ children }: { children: ReactNode }) {
  ...
  const mats = useMemo(() => {
    ...
    const value: WorldMats = {
      ...
      stone: std(maps.stone, "#bfc3c3", 0.92, 0.03),
      stoneDark: std(maps.stone, "#7b8285", 0.96, 0.03),
      rock: std(maps.rock, "#959da0", 0.96, 0.03),
      cliff: std(maps.cliff, "#a8adb1", 0.98, 0.03),
      canvas: Object.assign(std(maps.canvas, "#cdc3aa", 0.97), { side: THREE.DoubleSide }),
      roof: std(maps.wood, "#a49c8b", 0.94),
