...
export class MotionPlayback {
  ...
  ended = false;
  completions = 0;
  constructor(manifest: MotionManifest) {
    this.completions = 0;
    this.setMotion(manifest);
  }

  setMotion(manifest: MotionManifest, { preservePhase = false }: { preservePhase?: boolean } = {}) {
  ...
  advance(ms: number, { repeat = false }: { repeat?: boolean } = {}) {
  ...
  travel(distance: number, strideLength: number) {
...
export function isTravelMotion(manifest: MotionManifest | undefined) {
 ...
 */
export function motionPlacement(manifest: MotionManifest, bodyHeight: number) {
  ...
  if (typeof height !== "number" || !Number.isFinite(height) || height <= 0 || !Number.isFinite(bodyHeight) || bodyHeight <= 0 ||
...
export function reviewTravelVector(direction: string) {
